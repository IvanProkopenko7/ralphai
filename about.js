(() => {
  ...
  const i18n = {
    ...
    aboutP2: isPolish
      ? 'Model rozpoznaje 43 rodzaje metek: od szalików i czapek po garnitury i kurtki, włącznie z naprawdę rzadkimi metkami vintage z lat 70. i 80. Był trenowany na realnych ofertach z Vinted i eBay, więc dobrze radzi sobie ze zdjęciami nie najlepszej jakości — słabe światło, zły kąt czy rozmycie nie są problemem. Do każdego wyniku model dołącza poziom pewności, żebyś wiedział, ile możesz na nim polegać.'
      : 'The model recognises 43 types of labels: from scarves and hats to suits and jackets, including truly rare vintage labels from the 70s and 80s. It was trained on real listings from Vinted and eBay, so it copes well with photos of less-than-ideal quality — poor lighting, bad angles or blurriness are no problem. The model attaches a confidence level to each result so you know how much you can rely on it.',
    aboutP3: isPolish
